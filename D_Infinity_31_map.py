...
def Flag(arr, i, j):
    if i-1 >= 0 and j-1 >= 0 and i+1 < Ysize and j+1 < Xsize:
        ...
        try:
            ...
            if (h0 > h_around).any():
                return i, j, 0 #Dinfinity
            elif (h0 == h_around).any():
                return i, j, 1 #Flat
            else:
                return i, j, 2 #Sink
        except:
            return i, j, np.nan #Error
    else:
        return i, j, np.nan
...
def Receive(returnarr, result):
    i, j, x = result
    returnarr[i][j] = x

#calculation
result = map(Flag, myarray, np.ndindex(myarray.shape))
map(Receive, returnarrayF, result)
