...
・隣接する8セルへの流出がない「流出開始セル」に関して、ラスタの持つセル面積分だけ初期値を与えた
...
class FlowAccumulation(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        def StartCellScreaning(Input_FDarray, i, j): # 隣接する8セルへの流出がない「流出開始セル」と「流出中間セル」を選別し、「流出開始セル」にラスタの持つセル面積分だけ初期値を与えた
        ...
        def Accumulation(Input_FDarray, Output_FAarray, i, j):
            ...
            if Input_FDarray[i,j+1] <= 135 or Input_FDarray[i,j+1] >= 225:
                ...
                d += (Output_FAarray[i,j+1]+1)*((Input_FDarray[i,j+1]-135)/45)
            elif 135 < Input_FDarray[i,j+1] < 180 and Output_FAarray[i,j+1] == 0:
                d += (Input_FDarray[i,j+1]-135)/45
            elif 180 <= Input_FDarray[i,j+1] < 225 and Output_FAarray[i,j+1] > 0:
                d += (Output_FAarray[i,j+1]+1)*((225-Input_FDarray[i,j+1])/45)
            elif 180 <= Input_FDarray[i,j+1] < 225 and Output_FAarray[i,j+1] == 0:
                d += (225-Input_FDarray[i,j+1])/45
            ...
            if Input_FDarray[i-1,j+1] <= 180 or Input_FDarray[i-1,j+1] >= 270:
                ...
                d += (Output_FAarray[i-1,j+1]+1)*((Input_FDarray[i-1,j+1]-180)/45)
            elif 180 < Input_FDarray[i-1,j+1] < 225 and Output_FAarray[i-1,j+1] == 0:
                d += (Input_FDarray[i-1,j+1]-180)/45
            elif 225 <= Input_FDarray[i-1,j+1] < 270 and Output_FAarray[i-1,j+1] > 0:
                d += (Output_FAarray[i-1,j+1]+1)*((270-Input_FDarray[i-1,j+1])/45)
            elif 225 <= Input_FDarray[i-1,j+1] < 270 and Output_FAarray[i-1,j+1] == 0:
                d += (270-Input_FDarray[i-1,j+1])/45
            ...
            if Input_FDarray[i-1,j] <= 225 or Input_FDarray[i-1,j] >= 315:
                ...
                d += (Output_FAarray[i-1,j]+1)*((Input_FDarray[i-1,j]-225)/45)
            elif 225 < Input_FDarray[i-1,j] < 270 and Output_FAarray[i-1,j] == 0:
                d += (Input_FDarray[i-1,j]-225)/45
            elif 270 <= Input_FDarray[i-1,j] < 315 and Output_FAarray[i-1,j] > 0:
                d += (Output_FAarray[i-1,j]+1)*((315-Input_FDarray[i-1,j])/45)
            elif 270 <= Input_FDarray[i-1,j] < 315 and Output_FAarray[i-1,j] == 0:
                d += (315-Input_FDarray[i-1,j])/45
            ...
            if Input_FDarray[i-1,j-1] <= 270:
                ...
                d += (Output_FAarray[i-1,j-1]+1)*((Input_FDarray[i-1,j-1]-270)/45)
            elif 270 < Input_FDarray[i-1,j-1] < 315 and Output_FAarray[i-1,j-1] == 0:
                d += (Input_FDarray[i-1,j-1]-270)/45
            elif 315 <= Input_FDarray[i-1,j-1] and Output_FAarray[i-1,j-1] > 0:
                d += (Output_FAarray[i-1,j-1]+1)*((360-Input_FDarray[i-1,j-1])/45)
            elif 315 <= Input_FDarray[i-1,j-1] and Output_FAarray[i-1,j-1] == 0:
                d += (360-Input_FDarray[i-1,j-1])/45
            elif Input_FDarray[i-1,j-1] == -1:
                pass
            else:
                return False
            ...
            if Input_FDarray[i,j-1] >= 45 and Input_FDarray[i,j-1] <= 315:
                ...
                d += (Output_FAarray[i,j-1]+1)*(1-(Input_FDarray[i,j-1]/45))
            elif  0 <= Input_FDarray[i,j-1] < 45 and Output_FAarray[i,j-1] == 0:
                d += 1-(Input_FDarray[i,j-1]/45)
            elif 315 < Input_FDarray[i,j-1] and Output_FAarray[i,j-1] > 0:
                d += (Output_FAarray[i,j-1]+1)*((Input_FDarray[i,j-1]-315)/45)
            elif 315 < Input_FDarray[i,j-1] and Output_FAarray[i,j-1] == 0:
                d += (Input_FDarray[i,j-1]-315)/45
            elif Input_FDarray[i,j-1] == -1:
                pass
            else:
                return False
            ...
            if Input_FDarray[i+1,j-1] >= 90 or Input_FDarray[i+1,j-1] <= 0:
                ...
                d += (Output_FAarray[i+1,j-1]+1)*(Input_FDarray[i+1,j-1]/45)
            elif Input_FDarray[i+1,j-1] < 45 and Output_FAarray[i+1,j-1] == 0:
                d += Input_FDarray[i+1,j-1]/45
            elif 45 <= Input_FDarray[i+1,j-1] < 90 and Output_FAarray[i+1,j-1] > 0:
                d += (Output_FAarray[i+1,j-1]+1)*((90-Input_FDarray[i+1,j-1])/45)
            elif 45 <= Input_FDarray[i+1,j-1] < 90 and Output_FAarray[i+1,j-1] == 0:
                d += (90-Input_FDarray[i+1,j-1])/45
            elif Input_FDarray[i+1,j-1] == -1:
                pass
            else:
                return False
            ...
            if Input_FDarray[i+1,j] <= 45 or Input_FDarray[i+1,j] >= 135:
                ...
                d += (Output_FAarray[i+1,j]+1)*((Input_FDarray[i+1,j]-45)/45)
            elif 45 < Input_FDarray[i+1,j] < 90 and Output_FAarray[i+1,j] == 0:
                d += (Input_FDarray[i+1,j]-45)/45
            elif 90 <= Input_FDarray[i+1,j] < 135 and Output_FAarray[i+1,j] > 0:
                d += (Output_FAarray[i+1,j]+1)*((135-Input_FDarray[i+1,j])/45)
            elif 90 <= Input_FDarray[i+1,j] < 135 and Output_FAarray[i+1,j] == 0:
                d += (135-Input_FDarray[i+1,j])/45
            elif Input_FDarray[i+1,j] == -1:
                pass
            else:
                return False
            ...
            if Input_FDarray[i+1,j+1] <= 90 or Input_FDarray[i+1,j+1] >= 180:
                ...
                d += (Output_FAarray[i+1,j+1]+1)*((Input_FDarray[i+1,j+1]-90)/45)
            elif 90 < Input_FDarray[i+1,j+1] < 135 and Output_FAarray[i+1,j+1] == 0:
                d += (Input_FDarray[i+1,j+1]-90)/45
            elif 135 <= Input_FDarray[i+1,j+1] < 180 and Output_FAarray[i+1,j+1] > 0:
                d += (Output_FAarray[i+1,j+1]+1)*((180-Input_FDarray[i+1,j+1])/45)
            elif 135 <= Input_FDarray[i+1,j+1] < 180 and Output_FAarray[i+1,j+1] == 0:
                d += (180-Input_FDarray[i+1,j+1])/45
            elif Input_FDarray[i+1,j+1] == -1:
                pass
            else:
                return False
        ...
        for i, j in np.argwhere(myarrayFD != -1):
            returnarrayFA[i][j] = StartCellScreaning(myarrayFD, i, j)
        ...
        returnarrayFA == returnarrayFA * dxy
