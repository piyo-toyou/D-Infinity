...
layer_uri = input_layer.dataProvider().dataSourceUri()

#Get Array from Layer
input_src = gdal.Open(layer_uri, gdal.GA_ReadOnly)
input_raster = input_src.GetRasterBand(1)
input_array = input_raster.ReadAsArray()

#Get Raster Parameters
arr_height, arr_width = input_array.shape
geotransform = input_src.GetGeoTransform()
LeftUp_X = geotransform[0]
LeftUp_Y = geotransform[3]
dx = geotransform[1]
dy = geotransform[5]
dxy = sqrt(pow(dx,2)+pow(dy,2))
dtan = atan(dx/dy)
arr_d_type = input_array.dtype
...
for i in range(0, rows):
    for j in range(0, cols):
        args = c.DInfinity(input_array, i, j)
        returnarrayS[i][j] = args[0]
        returnarrayR[i][j] = args[1]

#Drive Output File
output_Path = "C:/Users/S2212357/Documents/Z9_Training/QGIS/WaterFlow/FlowDirection/FD_01.tiff"
output_driver = gdal.GetDriverByName("GTIFF")
output_raster = output_driver.Create(output_Path, arr_width, arr_height, 1, gdal.GDT_Float64)

#Output Array to Layer
output_raster.SetGeoTransform((LeftUp_X, dx, 0, LeftUp_Y, 0, dy))
output_band = output_raster.GetRasterBand(1)
output_band.WriteArray(returnarrayR)
